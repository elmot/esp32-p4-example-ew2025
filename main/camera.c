...
{
    //                  RRRRRGGGGGGBBBBB
    uint16_t color1 = 0b0010000100000100;
    uint16_t color2 = 0b1010010100010100;
    for (int y = 0; y < FRAME_V_RES; ++y)
    {
        if (y % 5 == 0)
        {
            uint16_t t = color1;
        ...
        for (int x = 0; x < FRAME_H_RES; x += 10)
        {
            for (int z = 0; z < 5; ++z)
            {
                cam_buffer[x + y * FRAME_H_RES + z] = color1;
                cam_buffer[x + y * FRAME_H_RES + z + 5] = color2;
            }
        }
    }
}
